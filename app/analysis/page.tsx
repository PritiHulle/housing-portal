    ...
    return (
        <div className="max-w-6xl mx-auto mt-8">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
                <div>
                    <h1 className="text-3xl font-extrabold text-white mb-2">Batch Property Analysis</h1>
                    <p className="text-zinc-400 text-sm">Analyze multiple properties simultaneously to compare values.</p>
                </div>
                <div className="flex gap-3">
                    <button
                        onClick={addRow}
                        className="flex items-center justify-center gap-2 px-5 py-2.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-100 rounded-xl border border-white/5 transition-all text-sm font-medium shadow-sm active:scale-95 whitespace-nowrap"
                    >
            ...
            <div className="bg-zinc-900/60 border border-white/5 rounded-2xl shadow-xl overflow-hidden backdrop-blur-xl mb-8 overflow-x-auto">
                <div className="min-w-[800px]">
                    {/* Table Header */}
                    <div className="grid grid-cols-8 gap-4 p-5 border-b border-white/10 bg-zinc-950/40 text-xs font-semibold text-zinc-400 uppercase tracking-wider">
                            ...
                            <div key={index} className="grid grid-cols-8 gap-4 items-center transition-all duration-300">
                                {FIELDS.map(field => (
                                    <input
                                        ...
                                        title={field.tooltip}
                                        required
                                        className="w-full bg-zinc-950/50 border border-white/10 rounded-lg px-3 py-2 text-sm text-zinc-100 placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-blue-500 focus:border-blue-500 transition-all font-mono"
                                    />
                                ))}
                                <div className="flex justify-end">
                                    <button
                                        ...
                                        className="p-2 text-red-500 bg-red-500/10 hover:text-red-400 hover:bg-red-500/20 rounded-lg transition-colors disabled:opacity-30 disabled:cursor-not-allowed group border border-red-500/20"
                                        title="Remove Row"
                                    >
            ...
            {results.length > 0 && (
                <div className="transition-all duration-500">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mb-8">
                        {results.map((val, i) => (
                            <div key={i} className="bg-gradient-to-br from-emerald-500/10 to-transparent border border-emerald-500/20 rounded-2xl p-5 shadow-lg isolate overflow-hidden relative group hover:border-emerald-500/40 transition-colors">
                                ...
                                <div className="text-zinc-400 text-xs font-medium mb-1 uppercase tracking-wider relative z-10">Property {i + 1}</div>
                                <div className="text-2xl font-bold tracking-tight text-emerald-400 relative z-10">
                    ...
                    <div className="bg-zinc-900/60 border border-white/5 rounded-2xl p-6 shadow-xl backdrop-blur-xl mb-12">
                        <h4 className="text-lg font-semibold text-white mb-6">Comparative Estimate Chart</h4>
