...
export default function MarketDashboard() {
    ...
    const handleRunSimulation = async () => {
        ...
        try {
            ...
            const res = await fetch("http://localhost:8080/api/market/what-if", {
                method: "POST",
    ...
    const fetchDashboardData = () => {
        ...
        fetch(`http://localhost:8080/api/market/stats${qs}`)
            .then(res => res.json())
            .then(data => setStats(data))
            .catch(err => console.error("Failed to connect to Java backend:", err));

        fetch(`http://localhost:8080/api/market/distribution`)
            .then(res => res.json())
            .then(data => {
